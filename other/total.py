...
# 진동센서 설정 : 핀
vibration_pin_num = 23  
GPIO.setup(vibration_pin_num, GPIO.IN) 
...
while True:
    ...
    # 진동 센서 데이터 처리
        cur_time = time.time()
        if GPIO.input(vibration_pin_num):
            vibration_count += 1

        if cur_time - prev_time > 5:
            if vibration_count > 20:
                GPIO.output(LED_PIN, GPIO.HIGH)
            else:
                GPIO.output(LED_PIN, GPIO.LOW)
            log_vibration_data(vibration_count)
            prev_time = time.time()
            vibration_count = 0
